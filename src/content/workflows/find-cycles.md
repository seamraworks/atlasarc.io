...
steps:
  ...
  - heading: Act on the classification.
    body: "For design problems: refactor the dependency relationship. Click any cycle participant to navigate directly to the class in the IDE, then invert the dependency, extract a shared interface, or move the code to the right layer. For intentional exceptions and tracked debt: choose Record cycle decision on the concrete dependency, classify it as Intentional or Debt, add the reason and optional ticket, review the exact governance-file diff, and commit that decision with the code. The dependency stays structurally visible but no longer keeps the governed cycle red. For out-of-scope noise during the current investigation: right-click the package in the graph and add a Safe Haven. AtlasArc suppresses that package's cycle participation for your current workspace so you can focus on the cycles that actually matter right now, without committing anything to the codebase."
metrics:
  - cyclic-dependencies
interpretation: |
