...
seo_description: Use AtlasArc to load a JaCoCo coverage report and find which load-bearing packages the test suite is not exercising, with coverage shown in architectural context.
problem: |
...
steps:
  - heading: Load a JaCoCo XML report.
    body: Generate a JaCoCo XML coverage report from your build and load it into AtlasArc. AtlasArc reads the coverage data and associates it with the packages in the analysis model.
